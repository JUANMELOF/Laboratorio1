...
class Reproductor():       
    def play(self,event):
        if not self.bandera:
            if not mx.music.get_busy(): 
                ...
                titulo_cancion = os.path.basename(self.canciones[self.numCancion])
                duracion_cancion = mx.Sound(self.canciones[self.numCancion]).get_length()
                self.lblestado.config(text=f"Reproduciendo: {titulo_cancion}")
    ...
    def pause(self,event):
        if self.bandera==False:
            ...
            titulo_cancion = os.path.basename(self.canciones[self.numCancion])
            duracion_cancion = mx.Sound(self.canciones[self.numCancion]).get_length()
            self.lblestado.config(text=f"pausado: {titulo_cancion}")
            self.btnPause.config(state="normal")
            self.btnStop.config(state="normal")
            self.btnPlay.config(state="disabled")
            self.bandera=True
    ...
    def cargarlista(self,event):
        ...
        self.listbox = Listbox(self.ventana, fg="white", bg="black")
        ...
        if filenames:
            ...
            for cancion in self.canciones:
                nombre_cancion = os.path.basename(cancion)  # Obtiene solo el nombre del archivo
                self.listbox.insert(END, nombre_cancion)
